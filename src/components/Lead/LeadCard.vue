            ...
            <v-row>
                ...
                <v-col cols="12" sm="10" md="10" lg="11">
                    <v-row>
                        ...
                        <br>
                        <span class="noPad caption grey--text">{{ getFormattedDate }}</span>
                    </v-row>
                </v-col>
            </v-row>
        </v-card-title>
        <div>
            ...
            <div v-if="lead.statusLeadId === 3">
                ...
                <div class="mt-2 pl-4 pb-2">
                    <v-row>
                        <v-col cols="12" sm="6" md="2" lg="2">
                            <v-btn class="pl-4 white--text" depressed color="#FF7B19" @click="changeStatus(1)">
                                Accept
                            </v-btn>
                        </v-col>
                        <v-col cols="12" sm="6" md="2" lg="2">

                            <v-btn class="pl-4 black--text" depressed color="#d3d3d3" @click="changeStatus(2)">
                                Decline
                            </v-btn>
                        </v-col>
...
export default class LeadCard extends Vue {
    ...
    public get getFormattedDate(){
        return this.formatarData(this.lead.dateCreated.toString());
    }

    public formatarData(dataString: string): string {
    ...
    const formattedDate = new Intl.DateTimeFormat('en-US', options).format(data).replaceAll('at','@');
        return formattedDate;
}
    public async changeStatus(status: number) {
